function [model,best_model]=train_test_model(model)
...
if ~isfield(model,'optimizer')
    disp('No optimizer was selected')
    disp('Setting SGD as the optimizer')
    model.optimizer='SGD';
end
for layeri=1:(length(model.layers))
    ...
    elseif strcmp(model.optimizer,'ADAM')
        ...
        disp('No proper optimizer was chosen.')
        disp('Setting SGD as the optimizer')
        model.optimizer='SGD';
    end
end
...
if (length(model.layers)>1) && (model.fnzl==length(model.layers))
    disp('All learning rates are zero. The model will be identical! No point continuing!');
    return;
end
...
for epoch=1:model.epochs
    ...
    if mod(epoch,model.update)==0
        ...
        if strcmp(model.type,'regression')
            disp(['Epoch: ' num2str(epoch) ' - Training: ' sprintf('%2.10f',model.error(epoch-1))]);
        else
            ...
            disp(['Epoch: ' num2str(epoch) ' - Training: ' sprintf('%2.2f',get_perf(out_train,model.y)) y_test_str]);
        end
    end
    ...
    [model,~]=forwardpassing_nolr(model,model.x(batchinds,:));
    ...
    for layeri=(length(model.layers)):-1:model.fnzl
        ...
        if layeri==length(model.layers)
            ...
            dedw=bsxfun(@times,permute(dedb,[2 3 1]),model.layers(layeri).X);
            
            mdedw=permute(mean(dedw,1),[2 3 1]);
            mdedw=mdedw(model.layers(layeri).nonzeroinds);
        else
            dedb=permute(mean(bsxfun(@times,permute(model.layers(layeri+1).grad,[2 3 1]),permute(model.layers(layeri+1).W,[3 1 2])),3).*model.layers(layeri).doutdnet,[2 1]);
            ...
            %mdedw=mdedw(model.layers(layeri).nonzeroinds);
            temp=permute(dedb,[3 1 2]).*permute(model.layers(layeri).X,[2 3 1]);
            mdedw=mean(temp,3);
            mdedw=mdedw(model.layers(layeri).nonzeroinds);
        ...
        elseif strcmp(model.optimizer,'ADAM')
            ...
            disp('Hey you have not chosen any optimizer!');
        end
